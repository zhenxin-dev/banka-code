...
- `Ctrl+O`：展开或收起工具活动详情；工具摘要始终保留在会话中。
- `↑` / `↓`：输入框为空时浏览本地提示历史；命令面板、审批和问题选项打开时用于移动选择。
- `Tab`：补全当前命令，`Enter`：发送提示或确认选项，`Esc`：中断当前运行或取消待确认操作。

空会话欢迎页会根据终端宽度自动切换双栏/窄屏布局，并展示当前的 Skills、MCP、LSP 能力；会话开始后底部状态栏继续显示工作区、Git 分支、Provider、模型、权限模式和上下文估算。
